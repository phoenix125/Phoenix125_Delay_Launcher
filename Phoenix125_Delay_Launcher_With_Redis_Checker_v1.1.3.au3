...
#AutoIt3Wrapper_Outfile=Builds\Phoenix125_Delay_Launcher_With_Redis_Checker_v1.1.3.exe
#AutoIt3Wrapper_Outfile_x64=Builds\Phoenix125_Delay_Launcher_With_Redis_Checker(x64).exe
...
#AutoIt3Wrapper_Res_ProductVersion=v1.1.3
...
Global $aIniShowConfigYNLine = "System use only. Do not change"
Global $aIniStartMinimizedYNLine = "Start programs minimized? (yes/no)"
...
For $x = 0 To ($aEntriesCount - 1)
	...
	For $i = 1 To $xDelay[$x]
		...
		Sleep(1000)
	Next
	If $aCheckForRedisYN = "yes" And $x = 1 Then
		Local $aStopLoop = False
		Do
			Local $i = 0
			For $i = 1 To 10
				If ProcessExists("redis-server.exe") Then
					LogWrite("Redis-server.exe found.")
					$aStopLoop = True
					ExitLoop
				Else
					ControlSetText($aSplashStartUp, "", "Static1", $aStartText & "Waiting for redis-server.exe to start." & @CRLF & "Countdown: " & (10 - $i + 1) & " seconds.")
					Sleep(1000)
				EndIf
			Next
			If $aStopLoop = False Then
				LogWrite("NOTICE! Redis-server.exe did not start within 10 seconds. Rerunning first program: " & $xFile[$x])
				If $aStartMinimizedYN = "no" Then
					Run($xFile[$x - 1], $aPathSplit[1] & $aPathSplit[2])
				Else
					Run($xFile[$x - 1], $aPathSplit[1] & $aPathSplit[2], @SW_MINIMIZE)
				EndIf
			EndIf
		Until $aStopLoop
	EndIf
	...
	Else
		Run($xFile[$x], $aPathSplit[1] & $aPathSplit[2], @SW_MINIMIZE)
	EndIf
Next
...
Func NoIniExist($tRestart = True)
	...
	$i += 1
	Global $aStartMinimizedYN = InputBox("Welcome to " & $aUtilName, "Question " & $i & " of " & $aTotalQuestionCount & @CRLF & $aIniStartMinimizedYNLine, "no", "", $aWidth, $aHeight)
	IniWrite($aIniFile, $aIniHeaderMain, $aIniStartMinimizedYNLine, $aStartMinimizedYN)
	MsgBox($MB_OK, $aUtilName, "Restarting to implement changes.", 2)
	IniWrite($aIniFile, $aIniHeaderMain, $aIniShowConfigYNLine, "no")
...
Func ReadUini()
	...
	Global $aShowConfigYN = IniRead($aIniFile, $aIniHeaderMain, $aIniShowConfigYNLine, $iniCheck)
	Global $aStartMinimizedYN = IniRead($aIniFile, $aIniHeaderMain, $aIniStartMinimizedYNLine, $iniCheck)
	...
	EndIf
	If $iniCheck = $aCheckForRedisYN Then
		$aCheckForRedisYN = "no"
		$iIniFail += 1
...
;~ 				Case $tInputSelectFile
	...
	UpdateIni(True)
	SplashOff()
	IniWrite($aIniFile, $aIniHeaderMain, $aIniShowConfigYNLine, "yes")
...
Func UpdateIni($aMakeBackupTF = True)
	...
	IniWrite($aIniFile, $aIniHeaderMain, $aIniCheckForRedisLine, $aCheckForRedisYN)
	IniWrite($aIniFile, $aIniHeaderMain, $aIniStartMinimizedYNLine, $aStartMinimizedYN)
	IniWrite($aIniFile, $aIniHeaderMain, $aIniShowConfigYNLine, $aShowConfigYN)
	FileWriteLine($aIniFile, @CRLF)
	IniWrite($aIniFile, $aIniHeaderMain, $aIniEntriesCountLine, $aEntriesCount)
	FileWriteLine($aIniFile, @CRLF)
	For $i = 0 To ($aEntriesCount - 1)
